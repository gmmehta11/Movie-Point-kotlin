...
class PopularMoviePagedListAdapter(public val context:Context): PagedListAdapter<Movie, RecyclerView.ViewHolder>(MovieDiffCallback()) {
    ...
    fun setNetworkState(newNetworkState: NetworkState){
        ...
        if(hadExtraRow != hasExtraRow){
            ...
            }else{                                          // hasExtrarow is true and hasExtraRow false
                notifyItemInserted(super.getItemCount())    //add the progressbar at the end
            }
        } else if(hasExtraRow && previousState != networkState){ // hasExtraRow is true and hadExtraRow true When Error occurs
